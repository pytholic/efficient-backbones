...
efficientnet_lite_params = {
    ...
    "efficientnet_lite0": [0.5, 0.5, 256, 0.2],
...
def round_filters(filters, multiplier, divisor=8, min_width=None):
    ...
    new_filters = max(
        min_width, int(filters + divisor / 2) // divisor * divisor
    )
...
class MBConvBlock(nn.Module):
    def __init__(
        self, inp, final_oup, k, s, expand_ratio, se_ratio, has_se=False
    ):
...
class EfficientNetLite(nn.Module):
    def __init__(
        ...
        mb_block_settings = [
            ...
            [1, 3, 1, 1, 32, 16, 0.25], # ? 16 -> 32 originally
            [2, 3, 2, 6, 16, 24, 0.25],
            [2, 5, 2, 6, 24, 40, 0.25],
            [3, 3, 2, 6, 40, 80, 0.25],
            [3, 5, 1, 6, 80, 112, 0.25],
        ...
        if global_layer == "GDC":
            self.global_layer = GDC(out_channels)
        elif global_layer == "GNAP":
            self.global_layer = GNAP(out_channels)
...
if __name__ == "__main__":
    ...
    net = build_efficientnet_lite(num_classes=1, name=model_name)
    net.eval()
